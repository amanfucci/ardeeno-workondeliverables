...
7. L'email inserita è già utilizzata da un altro account, appare la scritta "Email già utilizzata".
...
1. L'utente anonimo riceve un'email da *accounts.ardeeno@gmail.com* con oggetto "Conferma email".
...
3. La nuova email che è stata inserita è già presente nel sistema.
...
10. L'email è automaticamente confermata.
...
7. Il CF o l'email inserita sono già presenti nella BD, appare la scritta "Dipendente già registrato".
...
4.2.1 Gestione credenziali

Descrizione:
Questo componente si occupa delle pagine relative alla modifica delle credenziali dell’utente. E della visualizazione delle credenziali gia esistenti.

Interfaccia richiesta – modifica dati account.
Permette la modifica dei dati del utente. Email, numero di telefono, nome, cognome, indirizzo e informazioni bancarie.

Interfaccia richiesta – richiesta recupero password.
Facendo la richiesta si inizia il processo automatico che, sara dopo gestito dal componente di gestione Gmail.

Interfaccia richiesta – dati account.
Recupera le informazioni sul’account del cliente dal database.

Interfaccia richiesta – Conferma operazione.
Riceve la conferma dal componente ‘Authentication management’ che l’utente si e autenticato corretamente. Neccessario per permettere la modifica/visualizzazione dei dati personali che gestisce   questo componente.

Interfaccia fornita – richiesta recupero password.
Trasmette al componente ‘Gestione Gmail’ la richiesta di recupero password del’utente e il suo indirizzo email.

Interfaccia fornita – Mostra dati account.
Mostra al cliente una pagina con i suoi dati.

Interfaccia fornita – Nuovi dati account.
Modifica/aggiunge nel database i nuovi dati inseriti dall utente				
...
4.2.2 Gestione Gmail

Descrizione:
Il componente si occupa di gestire l’interfaccia con Gmail rispetto alle richieste di cambio password.

Interfaccia richiesta – richiesta recupero password.
Riceve dal componente ‘Gestione credenziali’ la richiesta di cambiare password di un utente e il suo indirizzo mail.

Interfaccia fornita – mail con codice cambio password.
Manda una mail al indirizzo email dell’utente con un codice generato dal componente necessario per potere cambiare password.

Interfaccia fornita – codice recupero password.
Trasmette al componente ‘Authentication management’ il codice di cambio password generato dal componente e l’utente associato.
...
Descrizione:
Il componente si occupa del login dell’utente, di modifiche password e di confermare l’identita dello utente agli altri componenti.

Interfaccia richiesta – codice recupero password.
Riceve eventuali codici di recupero password generati per utenti particolari dal componente ‘Gestione Gmail’

Interfaccia richiesta – form autenticazione.
Pagina di login di un utente richiede email e password dall’ utente.

Interfaccia richiesta –  password.
richiede al database la password associata a un account per verificare se il login è corretto.

Interfaccia richiesta –  codice recupero password (da utente).
Permette di ricevere email e codice di recupero password dall’utente, autorizzando cosi il cambiamento password.

Interfaccia richiesta –  nuova password.
Inserimento della nuova password.

Interfaccia fornita – nuova password.
trasmette al database la nuova password.

Interfaccia fornita – autentica cliente.
Questo componenete fornisce un interfaccia che permette di confermare l’utente, in quanto cliente, a tutti gli componenti che ne hanno bisogno.

Interfaccia fornita – autentica supervisore.
Questo componenete fornisce un interfaccia che permette di confermare l’utente in quanto cliente a tutti gli componenti che ne hanno bisogno.

Interfaccia fornita – autentica amministratore.
Questo componenete fornisce un interfaccia che permette di confermare l’utente in quanto cliente a tutti gli componenti che ne hanno bisogno.

Interfaccia fornita – autentica tecnico.
Questo componenete fornisce un interfaccia che permette di confermare l’utente in quanto cliente a tutti gli componenti che ne hanno bisogno.
...
4.2.4 pagine di presentazione

Descrizione:
Il componente si occupa di gestire la visualizzazione di tutte le pagine pubblicitarie o di presentazione dell’azienda a gli utenti non autenticati  

Interfaccia richiesta – pubblicita e modelli.
Prende dal database le nuove pubblicità e foto di modelli dell’azienda.

Interfaccia fornita – pubblicita è modelli.
Mostra all’utente la pubblicità.
...
4.2.5 Gestione impianti acquistati e acquisto impianti

Descrizione:
Il componente si occupa della gestione di tutto riguardante gli impianti di un utente. E lui che interfaccia con altri componenti per gestire sia l’acquisto di nuovi impianti che la visualizzazione di impianti individuali e i loro sensori.

Interfaccia richiesta – autenticazione del cliente.
Riceve da 4.2.3 l’identita del cliente autenticato in questione.

Interfaccia richiesta – richiesta visualizzazione singolo impianto.
Permette la selezione di un singolo impianto. Una volta selezionato la richiesta verra trasmessa a un altro componente (4.2.8).

Interfaccia richiesta – richiesta acquisto impianto.
Permette la selezione di un modello da acquistare. Una volta selezionato il componente verifichera la disponibilità. Se disponibile la richiesta vera trasmessa a un altro componente(4.2.6).

Interfaccia richiesta – conferma disponibilità.
Riceve conferma della disponibilità di un impianto per potere procedere con un acquisto.

Interfaccia richiesta – conferma acquisto impianto.
Riceve conferma del successo dell’acquisto incluso i dati necessari per aggiornare la lista degli impianti di cui è proprietario l’utente.


Interfaccia fornita – selezione impianto.
Trasmette al componente 4.2.8 la richiesta fatta dall’utente di visualizzare un certo impianto in particolare.

Interfaccia fornita – richiesta conferma disponibilità.
Richiesta di conferma della disponibilità di un particolare modello di sensore trasmessa al componente 4.2.7, necessaria prima di approvare un acquisto.

Interfaccia fornita – inizio transazione.
Se l’utente ha richiesto un acquisto e la sua disponibilità è confermata, le informazioni necessarie al acquisto sono trasmesse al componente 4.2.6
...
4.2.6 Gestione Paypal

Descrizione:
Il componente si occupa di interfacciare con paypal riguardo acquisti di impianti.

Interfaccia richiesta – inizio transazione.
Riceve informazioni riguardanti un acquisto da gestire.

Interfaccia richiesta – verifica disponibilità.
Verifica che la disponibilità del conto e permette la transazione.

Interfaccia fornita – invio acquisto.
Procede a richiedere a paypal di gestire l’acquisto.

Interfaccia fornita – Conferma acquisto.
Una volta completato un acquisto, l’informazione del successo deve essere trasmessa al componente 4.2.7.
...
4.2.7 Storehouse impianti

Descrizione:
Il componente si occupa di interfacciare con il database rispetto agli impianti in magazzino.

Interfaccia richiesta – richiesta conferma disponibilità.
Riceve da 4.2.5 richieste di confermare disponibilità di modelli in magazzino.

Interfaccia richiesta – Conferma acquisto
riceve da 4.2.6 conferma che un dato impianto è stato acquistato.

Interfaccia richiesta – scarica/aggiorna impianti in magazzino
Quando troppo tempo e passato dall’ultima volta il componente si aggiorna con il database per potere registrare l’arrivo di nuovi impianti disponibili.

Interfaccia fornita – conferma disponibilita
Trasmette conferma della disponibilità di un impianto da acquistare e metterà l’impianto in questione in uno stato prenotato finche riceverà la conferma dell’acquisto o sara scaduto il tempo della prenotazione.

Interfaccia fornita – conferma acquisto impianto
Quando un acquisto è stato confermato il componente invierà la conferma dell’acquisto e i dati dello impianto in questione al componente 4.2.5.

Interfaccia fornita – registra impianto acquistato
Trasmette al database le informazione riguardo l’acquisto. Per potere registrare anche lì che non e piu disponibile.
...
4.2.8 Gestione singolo impianto e i suoi sensori

Descrizione:
Il componente si occupa di gestire l’interfaccia con l’utente riguardo i dati di un singolo impianto e i suoi sensori.

Interfaccia richiesta – selezione impianto.
Riceve da 4.2.5 l’impianto selezionato da un utente.

Interfaccia richiesta – richiedi riparazione.
Il componente permette di riempire un form richiedendo un qualche tipo di intervento su l’impianto selezionato.

Interfaccia fornita – storico interventi.
Il componente mostra uno storico di tutti gli interventi conclusi sull’impianto (installazione/riparazione/disinstallazione).

Interfaccia fornita – visualizza misurazioni.
Permette la visualizzazione di tutti i dati generati dai sensori del impianto e statistiche su di essi.

Interfaccia fornita – richiesta riparazione.
Inoltra la richiesta di riparazione inserita dall’utente a un supervisore.

## Diagramma dei componenti

![](./.media/diagrammacomp.drawio.png){width=12cm fig-pos=H}