...
6. Il sistema -- tramite Gmail -- invia l'email contenente il codice per confermare l'indirizzo email.
...
6. L'acquisto è concluso, il sistema inserisce un intervento di installazione per il nuovo impianto ed -- tramite Gmail -- invia una richiesta di recensione per email.
...
![Activity Diagram](./.media/supactivity.drawio.png){width=10cm fig-pos=H}
...
11. La registrazione è stata correttamente effettuata, appare la scritta "Registrazione completata".
...
3. L'amministratore invia il form premendo il bottone "Modifica dati". [Exception 1]
...
***Descrizione***
Questo componente si occupa della funzione di visualizzazione della presentazione dell'azienda, che comprende descrizione testuale e mappa con sedi dell'azienda. La mappa è reperita da OpenStreetMap.
...
|Richiesta|Mappe|Le mappe fornite da OpenStreetMap, su cui saranno poste le sedi dell'azienda|
...
***Descrizione***
Questo componente si occupa di reperire le service reviews dal sistema esterno TrustPilot, e fornirle al componente "Pagina Presentazione".
...
|Richiesta|Service Reviews|Il componente attende da TrustPilot la lista di service reviews|

***Interfacce interne***

|Tipo|Nome|Descrizione|
|---|---|---------------------|
|Fornita|Service reviews|Il componente fornisce le service reviews ottenute da TrustPilot e trasformate in un formato comprensibile agli altri componenti|

### Gestione invio email

***Descrizione***
Questo componente si occupa di interagire col sistema esterno Gmail, a cui richiede di inviare le email fornite dalle altre componenti.

***Interfacce interne***

|Tipo|Nome|Descrizione|
|---|---|---------------------|
|Richiesta|Email|Il componente attende dagli altri componenti una email da inviare. L'email è composta da destinatari, oggetto, corpo e file allegati|

***Interfacce relative a Gmail***

|Tipo|Nome|Descrizione|
|---|---|---------------------|
|Fornita|Richiesta invio email|Il componenete fornisce al sistema Gmail l'API Token e l'email da inviare|

### Conferma indirizzo email

***Descrizione***
Questo componente si occupa dell'intero processo di conferma dell'indirizzo email: generazione del codice di conferma, invio dell'email con il codice di conferma e verifica del codice di conferma fornito dall'Utente Anonimo.
...
|Fornita|Email|Il componenete fornisce al comp. "Gestione invio email" l'email da inviare (che contiene il codice di conferma generato)|

***Interfacce relative ad Ardeeno DataLayer***

|Tipo|Nome|Descrizione|
|---|---|---------------------|
|Fornita|Richiesta inserimento codice|Il componente fornisce al DataLayer una richiesta di inserimento del codice di conferma generato; la richiesta comprende il codice di conferma e l'email da confermare|
...
|Richiesta|Esito Verifica|Il componente ottiene da Ardeeno DataLayer l'esito della verifica del codice di conferma|
...
|Fornita|Esito conferma|Il componente fornisce all'utente anonimo l'esito della conferma; se il codice è non esistente o scaduto l'esito è negativo, altrimenti l'email è confermata|

### Recupero password

***Descrizione***
Questo componente si occupa dell'intero processo di recupero della password: generazione del codice di recupero, invio dell'email con il codice di recupero, verifica del codice di conferma fornito dall'utente anonimo e inserimento della nuova password.
...
|Tipo|Nome|Descrizione|
|---|---|---------------------|
|Richiesta|Richiesta generazione codice recupero|Il componente attende dall'Utente Anonimo una richiesta di generazione del codice di recupero. La richiesta comprende un indirizzo email|
...
***Descrizione***
Questo componente gestisce il processo di registrazione di un cliente nel sistema.
...
***Descrizione***
Questo componente si occupa della funzione di login. In sintesi, si tratta di un componente cuscinetto tra l'Utente Anonimo ed il componente "Autenticazione".
...
***Descrizione***
Questo componente si occupa dell'autenticazione e comunica col sistema DataLayer. Inoltre è utilizzato dagli altri componenti per verificare lo stato di autenticazione.
...
***Descrizione***
Questo componente si occupa della gestione dell'account di un Utente Autenticato; ovvero permette a un Utente Autenticato di visualizzare e modificare i propri dati. Interagisce col comp. "Autenticazione". Inoltre, interagisce anche col comp. "Conferma email" nel caso in cui l'Utente modifichi il proprio indirizzo email.
...
***Descrizione***
Questo componente si occupa di gestire i pagamenti tramite PayPal.
...
***Descrizione***
Questo componente si occupa di gestire il processo di acquisto di un impianto da parte di un Cliente.
...
|Richiesta|Codice prod.review|Il componenete attende dal comp. "Gestione codice prod.review" il codice generato|
|Fornita|Email|Il componente fornisce al comp. "Gestione invio email" un'email contenente il codice prod.review, da inviare al Cliente che ha correttamente concluso l'acquisto|
...
***Descrizione***
Questo componente si occupa di richiedere e ricevere da TrustPilot i codici prod.reviews.
...
|Tipo|Nome|Descrizione|
|---|---|---------------------|
|Fornita|Richiesta codice prod.review|Il componente fornisce a TrustPilot la richiesta di un nuovo codice prod.review, che comprende: ID cliente, ID ordine, API token|
...
***Descrizione***
Questo componente si occupa di inserire automaticamente le richieste di intervento di installazione (all'acquisto di un impianto) e di manutenzione (periodicamente).
...
***Descrizione***
Questo componente si occupa di gestire la visualizzazione da parte dell'utente anonimo dei modelli di impianto in vendita e delle prod.reviews; questo componente si occupa inoltre di inoltrare la richiesta di acquisto al comp. "Acquisto impianto".
...
|Richiesta|Selezione modello|Il componenete attende che l'Utente Anonimo fornisca l'impianto selezionato|
...
***Descrizione***
Questo componente si occupa di reperire da TrustPilot le prod.reviews.
...
***Descrizione***
Questo componente permette ad un Utente Anonimo di visualizzare e selezionare gli impianti acquistati. Interagisce quindi con il sistema Ardeeno DataLayer.
...
|Tipo|Nome|Descrizione|
|---|---|---------------------|
|Richiesta|Check auth|Il componente ottiene dal comp. "Autenticazione" lo stato di autenticazione, che comprende anche l'ID Cliente e l'indirizzo email|
...
***Descrizione***
Questo componente si occupa di reperire dal DataLayer le misurazioni di un certo impianto, inoltrarle ad altri componenti e mostrarle al Cliente/Tecnico in forma tabulare. 
...
|Richiesta|Check auth|Il componente ottiene dal comp. "Autenticazione" lo stato di autenticazione; procede nel compiere le sue funzioni solo se si è in uno stato autenticato e se l'ID cliente/tecnico ha accesso al determinato ID impianto|
...
|Richiesta|Misurazioni|Il componente attende che sistema DataLayer fornisca le misurazioni richieste|
...
***Descrizione***
Questo componente si occupa di costruire la Heatmap e fornirla al Cliente, utilizzando le mappe fornite da OpenStreetMap e le misurazioni fornite dal comp. "Misurazioni". Inoltre, il Cliente interagisce con Il componente fornendo parametri e snapshot di interesse. 
...
Nota: per scelta architetturale entrambi i componenti ["Heatmap"](#heatmap) ed ["Pagina presentazione"](#pagina-presentazione) sono intrisecamente dipendenti da OpenStreetMap.
...
|Fornita|Heatmap|Il componente costruisce e fornsice al Cliente la Heatmap|
...
***Descrizione***
Questo componente fornisce al Cliente gli interventi relativi ad un impianto e permette al Cliente di inserire una richiesta di riparazione.
...
Nota: Il controllo sull'esistenza della coppia (ID impianto, ID cliente) è implicito nella richiesta inoltrata al DataLayer; una risposta vuota -- sia dal DataLayer che verso il Cliente -- puà essere interpretata come coppia (ID impianto, ID cliente) non esistente. Similmente per richieste al DataLayer nei componenti definiti precedentemente e successivamente.

### Pagina interventi Tecnico

***Descrizione***
Questo componente si occupa di raccogliere dal DataLayer gli interventi assegnati ad un certo Tecnico e fornirli al Tecnico.
...
Nota: per scelta architetturale molti componenti sono intrisecamente dipendenti anche ad Ardeeno DataLayer; sarà il sistema esterno paritario ad assicurarsi di mantenere un certo formato standard interno per richieste/risposte, anche al variare delle tecnologie e servizi utilizzati.
...
***Descrizione***
Questo componente interagisce col sistema GPS e fornisce internamente le coordinate GPS reperite.
...
***Descrizione***
Questo componente gestisce le interazioni tra il Tecnico e l'intervento assegnato selezionato: aggiunta/rimozione sensore, conclusione intervento.
...
|Richiesta|Check auth|Il componente ottiene dal comp. "Autenticazione" lo stato di autenticazione; procede nel compiere le sue funzioni solo se si è in uno stato autenticato e se l'ID Tecnico ha accesso al determinato ID intervento|
|Fornita|Richiesta coordinate|[Comp. "Gestioen GPS"](#gestione-gps)|
|Richiesta|Risposta cordinate|[Comp. "Gestioen GPS"](#gestione-gps)|
...
***Descrizione***
Questo componente si occupa di raccogliere dal DataLayer tutti gli interventi fornirli al Supervisore.
...
***Descrizione***
Questo gestisce l'assegnazione da parte di un Supervisore di un intervento richiesto a un certo Tecnico.
...
***Descrizione***
Questo componente gestisce la registrazione e la modifica degli account di Dipendenti da parte dell'Amministratore. Interagisce con molti altri componenti interni.
...
|---|---|---------------------|
|Richiesta|Check auth|Il componente ottiene dal comp. "Autenticazione" lo stato di autenticazione; procede nel compiere le sue funzioni solo se si è in uno stato autenticato|
|Fornita|Email|Il componente fornisce al comp. "Gestione invio email" un'email da inviare al Dipendente registrato/modificato contenente i dati dell'account|
|Fornita|Richiesta generazione codice di conferma|Il componente fornisce al comp. "Gestione conferma email" una richiesta di generazione di codice di conferma; questo avviene se si registra un Dipendente o se si modifica l'indirizzo email di un Dipendente|
...
|Fornita|Esito richiesta|Il componente fornisce all'Amministratore l'esito della richiesta di registrazione/modifica. L'esito è negativo se un'indirizzo email o un CF inserito è gia associato a qualche account|
...
|Fornita|Richiesta registrazione/modifica Dipendente|Il componente fornisce al sistema DataLayer la richiesta di registrazione/modifica|
|Richiesta||Il componente attende dal sistema DataLayer l'esito della richiesta di registrazione/modifica|

### Statistiche

***Descrizione***
Questo componente si occupa di raccogliere dati da Ardeeno DataLayer, aggregarli e fornire grafici interattivi all'Amministratore
...
|Tipo|Nome|Descrizione|
|---|---|---------------------|
|Richiesta|Richiesta grafico|Il componente attende che l'Amministratore fornisca una richiesta grafic, che comprende: tipo di grafico, periodo da considerare, parametri di interesse|
|Fornita|Grafic|Il componente costruisce e fornisce all'Amministratore il grafico interattivo|
...
![Anteprima diagramma dei componenti](./.media/diagrammacomp.drawio.png){fig-pos=H}