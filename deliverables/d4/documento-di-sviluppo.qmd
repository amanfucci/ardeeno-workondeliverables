...
![legenda](.media/user-flows/legenda.png){fig-pos=H width=5cm}
...
Infine, l'applicazione è stata sviluppata pensando ad un uso sia su Desktop che Mobile (Tablet, Smartphone) -- dunque nel concreto utilizzando le corretti componenti per garantire una visualizzazione ottimale dei contenuti per ogni grandezza di schermo (`Row`, `Col` con impostazioni su `xs.cols`, `md.cols`), e differenziando il motore di rendering della `Heatmap.js` a seconda del tipo di dispositivo (Mobile, Desktop); l'applicazione è stata quindi testata su Desktop (Ubuntu 20.04, Firefox 108), Tablet (Android 12, Firefox 108) ed Smartphone (Android 9, Firefox 108).
...
/* ·> /404
```
[^1]: Con `react-router 6.3`, le routes sono *final-slash insensitive*, ovvero /home ed /home/ sono sempre equivalenti.

Si descrivono ora le componenti sviluppate/modificate ad-hoc per `ardeeno-frontend`. 

### App.js
Si utilizza `BrowserRouter` anziché `HashRouter` e si configura un AppContext `React` -- si preferisce per semplicità utilizzare `localforage` anziché gli store `redux`. Si esporta la costante `API_URL` poiché la variabile di ambiente `REACT_APP_API_URL` è visibile esclusivamente in `App.js`.
...
### Home.js
Componenente sviluppata per implementare [RF1 Visualizzazione Presentazione](#a); si utilizza `react-leaflet` per mostrare la mappa con le sedi dell'azienda, connettendosi a un TileProvider OSM.
...
### MyImpianti.js
Componente sviluppata per implementare [RF11 Visualizzazione impianti acquistati](#a). Al primo accesso, per accedere alla Dashboard ed alla Heatmap è necessario selezionare un Impianto cliccando sul link `Go to Dashboard` -- se ciò non è fatto, la Dashboard e la Heatmap fanno un redirect ad `/MyImpianti/list`.
